...
export default {
  ...
  methods: {
    ...
    deletePopup(item, button) {
      ...
      this.infoModal.content = `Are you sure to delete this colum with id= ${item.IdColor}`;
      this.$root.$emit("bv::show::modal", this.infoModal.id, button);
      this.deleteItemId = item.IdColor;
    },
    ...
    openEdit(row) {
      if (row) {
        ...
        this.itemsEdit.IdColor = 0;
        this.itemsEdit.Color1 = "";
        this.itemsEdit.Name = "";
        this.isShowFormGroup = true;
      }
    },
    ...
    saveEditColor() {
      this.queryUrl = this.itemsEdit.IdColor
        ? this.urlDefault + `/${this.itemsEdit.IdColor}`
        : this.urlDefault;
      this.setByProp({
        queryUrl: this.queryUrl,
        payload: this.itemsEdit,
      });
      if (this.itemsEdit.IdColor) {
        this.putApi()
          .then((response) => {
            var indexOf = this.items.findIndex(
              (x) => x.IdColor == response.IdColor
            );
            this.items[indexOf].Color1 = response.Color1;
