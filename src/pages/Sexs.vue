...
export default {
  ...
  methods: {
    ...
    deletePopup(item, button) {
      ...
      this.infoModal.content = `Are you sure to delete this colum with id= ${item.SexId}`;
      this.$root.$emit("bv::show::modal", this.infoModal.id, button);
      this.deleteItemId = item.SexId;
    },
    ...
    saveEditSex() {
      this.queryUrl = this.itemsEdit.SexId
        ? this.urlDefault + `/${this.itemsEdit.SexId}`
        : this.urlDefault;
      this.setByProp({
        queryUrl: this.queryUrl,
        payload: this.itemsEdit,
      });
      if (this.itemsEdit.SexId) {
        this.putApi()
          .then((response) => {
            var indexOf = this.items.findIndex(
              (x) => x.SexId == response.SexId
            );
